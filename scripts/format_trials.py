...
if __name__ == "__main__":
    ...
    trials = np.loadtxt(args.src_trials_path, dtype=str)  #测试集标签

    f = open(args.dst_trials_path, "w")
    for item in trials:
        enroll_path = os.path.join(
            args.voxceleb1_root, item[1])  #第一个
        test_path = os.path.join(args.voxceleb1_root, item[2])  #第二个
        f.write("{} {} {}\n".format(item[0], enroll_path, test_path))  #写道vo1.txt里
