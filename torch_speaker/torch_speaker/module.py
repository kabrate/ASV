...
import torch_speaker.torch_speaker.audio as audio
import torch_speaker.torch_speaker.backbone as backbone
import torch_speaker.torch_speaker.loss as loss
import torch_speaker.torch_speaker.score as score
from torch_speaker.torch_speaker.audio import Evaluation_Dataset, Train_Dataset
from torch_speaker.torch_speaker.utils import count_spk_number
...
class Task(LightningModule):
    def __init__(self, **kwargs):
        ...
        self.feature = getattr(audio, feature_name)(**self.hparams.feature)  #160采样为一帧 400采样为加窗长度  返回一个对象属性值
    ...
    def test_step(self, batch, batch_idx):
        x, path = batch
        path = str(path[0]) #x 每次采样的数据量=总数据量除以每次采样的数据量
