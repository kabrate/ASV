...
class ResNet(nn.Module):
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        # See note [TorchScript super()] # 1 1 80 681
        x = self.conv1(x) #1 64 80 681      output 64 channel  per feture 80 dimension  total 681 frame
        x = self.bn1(x)
        x = self.relu(x)

        x = self.layer1(x)# 1 32 80 681
        x = self.layer2(x)#1 64 40 341
        x = self.layer3(x)#1 128 20 171
        x = self.layer4(x)#1 256 10 86

        x = x.reshape(x.shape[0], -1, x.shape[-1])#1 2560 86
